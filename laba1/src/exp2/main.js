...
const resultDisplay = document.getElementById("typeInfoOutput");
userForm.addEventListener("submit", (event) => {
    event.preventDefault();
    // Получаем данные из формы
    const userName = document.getElementById("userNameInput").value;
    const userAge = parseInt(document.getElementById("userAgeInput").value);
    // Демонстрация различных типов данных
    const isRegisteredUser = true; // Логический тип (boolean)
    const emptyValue = null; // Значение null
    let undefinedVariable; // Неопределённое значение
    let typeInformation = "";
